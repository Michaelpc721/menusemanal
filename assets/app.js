...
let PRICE_DB = PRICE_DB_DEFAULTS; // fallback
let FAMILY_NAME_DEFAULTS_OVERRIDE = FAMILY_NAME_DEFAULTS; // fallback

async function loadConfig() {
  const response = await fetch('config-ejemplo.json');
  const config = await response.json();
  if (config.priceDB) {
    PRICE_DB = config.priceDB;
  }
  if (config.familyNames) {
    FAMILY_NAME_DEFAULTS_OVERRIDE = config.familyNames;
  }
}
