...
const CONFIG_FILE_URL = './config-ejemplo.json';
...
function parseConfigPayload(parsed) {
  return {
...
function loadConfigFromStorage() {
  try {
    ...
    if (!raw) return null;
    return parseConfigPayload(JSON.parse(raw));
  } catch (err) {
    console.warn('No se pudo leer la configuración guardada en este navegador.', err);
    return null;
  }
}

async function loadConfigFromFile() {
  const response = await fetch(CONFIG_FILE_URL, { cache: 'no-store' });
  if (!response.ok) {
    throw new Error(`No se pudo cargar ${CONFIG_FILE_URL} (HTTP ${response.status}).`);
  }
  return parseConfigPayload(await response.json());
}

function mergeConfigs(baseConfig, overrideConfig) {
  const base = baseConfig || buildDefaultConfig();
  const override = overrideConfig || {};
  return {
    version: Number(override.version) || Number(base.version) || 1,
    familyNames: normalizeFamilyNames({ ...(base.familyNames || {}), ...(override.familyNames || {}) }),
    priceDB: normalizePriceDB({ ...(base.priceDB || {}), ...(override.priceDB || {}) })
  };
}

let repoConfig = buildDefaultConfig();
let appConfig = buildDefaultConfig();
let familyNames = clone(FAMILY_NAME_DEFAULTS);
let priceDB = clone(PRICE_DB_DEFAULTS);
...
function resetAppConfig() {
  familyNames = clone(repoConfig.familyNames || FAMILY_NAME_DEFAULTS);
  priceDB = clone(repoConfig.priceDB || PRICE_DB_DEFAULTS);
  saveConfig();
  renderDays();
...
function resetPriceDB() {
  priceDB = clone((repoConfig && repoConfig.priceDB) || PRICE_DB_DEFAULTS);
  saveConfig();
  renderShopping();
}
...
async function init() {
  try {
    repoConfig = await loadConfigFromFile();
  } catch (err) {
    console.warn('No se pudo cargar la configuración base desde el JSON del repositorio. Se usarán los valores por defecto.', err);
    repoConfig = buildDefaultConfig();
  }

  const storedConfig = loadConfigFromStorage();
  appConfig = mergeConfigs(repoConfig, storedConfig);
  familyNames = clone(appConfig.familyNames || FAMILY_NAME_DEFAULTS);
  priceDB = clone(appConfig.priceDB || PRICE_DB_DEFAULTS);
